...
g44_   :=   -(-a^4*sin(theta)^2+a^4+2*r^2*a^2-a^2*sin(theta)^2*r^2+2*a^2*sin(theta)^2*M*r+r^4)*sin(theta)^2/(-a^2+sin(theta)
^2*a^2-r^2)   :
Info_ := `         Kerr metric (Boyer-Lindquist coordinates (J. Math. Phys. 8 265))        `: 
